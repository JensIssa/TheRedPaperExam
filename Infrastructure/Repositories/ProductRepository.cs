...
public class ProductRepository : IProductRepository
{
    ...
    {
        return _context.ProductTable.Where(i => i.SubCategoryID.Equals(subcategoryId)).Include
            (x => x.SubCategory).ToList();
    }

    public List<Product> GetAllProductsFromUser(int userId)
    {
        return _context.ProductTable.Where(i => i.userId.Equals(userId)).Include(x => x.user).ToList();
    }
